...
export default {
  get_list: (req, res, next) => {
    ...
    try {
      ...
      contentsService.get_list(param).then(data => {
        ...
        res.header('Content-Range', `contentsController ${range}/${data.count}`);
        res.send(dataOutput);
        // write log
  ...
  get_list_multi: (req, res, next) => {
    ...
    try {
      ...
      contentsService.get_list_multi(param).then(data => {
        const dataOutput = {
          points: data,
          success: true,
          errors: [],
  ...
  get_one: (req, res, next) => {
    ...
    try {
      ...
      contentsService.get_one(param).then(data => {
        const objLogger = loggerFormat(req, res);
  ...
  get_one_for_web: (req, res, next) => {
    recordStartTime.call(req);
    try {
      const { id } = req.params;
      let { attributes, range } = req.query;
      range = range ? JSON.parse(range) : [0, 49];
      const param = { id, attributes, range }

      // console.log("districtsService param: ", param)
      contentsService.get_one_for_web(param).then(data => {
        const objLogger = loggerFormat(req, res);
  ...
  create: (req, res, next) => {
    ...
    try {
      ...
      contentsService.create(param).then(data => {
        const objLogger = loggerFormat(req, res);
  ...
  update: (req, res, next) => {
    ...
    try {
      ...
      contentsService.update(param).then(data => {
        const objLogger = loggerFormat(req, res);
  ...
  update_status: (req, res, next) => {
    ...
    try {
      ...
      contentsService.update_status(param).then(data => {
        if (data && data.result) {
          const dataOutput = {
            result: data.result,
            success: true,
  ...
  updateShares: (req, res, next) => {
    ...
    try {
      ...
      contentsService.updateShares(param).then(data => {
        if (data && data.result) {
          const dataOutput = {
            result: data.result,
            success: true,
