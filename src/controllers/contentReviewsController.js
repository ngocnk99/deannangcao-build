import contentReviewsService from '../services/contentReviewsService'
...
export default {
  get_list: (req, res, next) => {
    ...
    try {
      ...
      contentReviewsService.get_list(param).then(data => {
        ...
        res.header('contentReview-Range', `contentReviewsController ${range}/${data.count}`);
        res.send(dataOutput);
        // write log
  ...
  get_list_for_web: (req, res, next) => {
    ...
    try {
      const { sort, range, filter, attributes } = res.locals;
      if (!filter.status || filter.status === 0) filter.status = 1;
      const param = {
        sort,
        range,
        filter,
      ...
      contentReviewsService.get_list_for_web(param).then(data => {
        const objLogger = loggerFormat(req, res);
        const dataOutput = {
          result: {
            list: data.rows,
  ...
  get_one: (req, res, next) => {
    ...
    try {
      ...
      contentReviewsService.get_one(param).then(data => {
        const objLogger = loggerFormat(req, res);
  ...
  create: (req, res, next) => {
    ...
    try {
      ...
      contentReviewsService.create(param).then(data => {
        const objLogger = loggerFormat(req, res);
  ...
  update_status: (req, res, next) => {
    ...
    try {
      ...
      contentReviewsService.update_status(param).then(data => {
        if (data && data.result) {
          const dataOutput = {
            result: data.result,
            success: true,
  ...
  bulkRemove: (req, res, next) => {
    recordStartTime.call(req);
    try {
      const filter = res.locals.filter;
      // const entity = req.body

      contentReviewsService.bulk_remove(filter).then(data => {
        if (data && data.result) {
          const dataOutput = {
            result: data.result,
            success: true,
