      ...
      MODELS.findOne(provinces,
        ...
        if (!findEntity) {
          ...
          MODELS.update(provinces,
            ...
            MODELS.findOne(provinces,{ where: { id: param.id } }).then(result => {
              if (!result) {
                reject(new ApiErrors.BaseError({
                  statusCode: 202,
                  type: 'deleteError',
  ...
  delete: async param => {
    try {
      ...
      if (!foundProvince) {
        ...
        await MODELS.destroy(provinces,
          { where: { id: parseInt(param.id) } }
        );

        const provinceAfterDelete = await MODELS.findOne(provinces,{ where: { Id: param.id } })
          .catch(err => {
