...
// typeOfnews
function makeArraySearch_typeOfnews(array, arrTreeId) {
  return new Promise(async (resolve, reject) => {
    try {
      arrTreeId = typeof arrTreeId !== 'undefined' ? arrTreeId : [];
      await Promise.all(
        array.map(async child => {
          arrTreeId.push(child.id);
          console.log('child.id====', child.id);
          if (child.typeOfNewParentId) {
            await makeArraySearch([{ id: child.typeOfNewParentId }], arrTreeId);
          }
        })
      );
...
function makeTreeArrayParentSearch_typeOfnews(array, arrTreeId, model, option) {
  return new Promise(async (resolve, reject) => {
    try {
      arrTreeId = typeof arrTreeId !== 'undefined' ? arrTreeId : [];
      await Promise.all(
        array.map(async child => {
          ...
          if (child.typeOfNewParentId) {
            const dataListChild = await Model.findAll(model, {
              where: { id: child.typeOfNewParentId, ...option },
            ...
            // console.log("arrayChild=======",arrayChild)
            await makeTreeArrayParentSearch_typeOfnews(arrayChild, arrTreeId, model, option);
          }
        })
      );
...
function makeTreeArrayChildSearch_typeOfnews(array, arrTreeId, model, option) {
  return new Promise(async (resolve, reject) => {
    try {
      arrTreeId = typeof arrTreeId !== 'undefined' ? arrTreeId : [];
      option = option || {};
      await Promise.all(
        array.map(async child => {
          ...
          if (child.id) {
            const dataListChild = await Model.findAll(model, {
              where: { typeOfNewParentId: child.id, ...option },
              attributes: ['id', 'typeOfNewName', 'typeOfNewParentId']
            });
            ...
            // console.log("arrayChild=======",arrayChild)
            await makeTreeArrayChildSearch_typeOfnews(arrayChild, arrTreeId, model, option);
          }
        })
      );
...
function makeTreeParentChildrenArraySearch_typeOfnews(array, arrTreeId, model, option) {
  return new Promise(async (resolve, reject) => {
    try {
      arrTreeId = typeof arrTreeId !== 'undefined' ? arrTreeId : [];
      option = option || {};
      ...
      await Promise.all(
        array.map(async child => {
          if (!_.includes(arrTreeId, child.id)) {
            ...
            if (child.typeOfNewParentId || Number(child.typeOfNewParentId) !== 0) {
              const data = await Model.findOne(model, {
                where: { id: child.typeOfNewParentId, ...option },
                attributes: ['id', 'typeOfNewName', 'typeOfNewParentId']
              });
            ...
            await makeTreeParentChildrenArraySearch_typeOfnews(arrayChild, arrTreeId, model, option);
          }
        })
      );
...
// botTypeOfPost
function makeTreeArrayParentSearch_botTypeOfPosts(array, arrTreeId, model, option) {
  return new Promise(async (resolve, reject) => {
    try {
      arrTreeId = typeof arrTreeId !== 'undefined' ? arrTreeId : [];
      await Promise.all(
        array.map(async child => {
          ...
          if (child.typeOfPostsParentId) {
            const dataListChild = await Model.findAll(model, {
              where: { id: child.typeOfPostsParentId, ...option },
              attributes: ['id', 'typeOfPostsName', 'typeOfPostsParentId']
            });
            ...
            // console.log("arrayChild=======",arrayChild)
            await makeTreeArrayParentSearch_botTypeOfPosts(arrayChild, arrTreeId, model, option);
          }
        })
      );
...
function makeTreeArrayChildSearch_botTypeOfPosts(array, arrTreeId, model, option) {
  return new Promise(async (resolve, reject) => {
    try {
      arrTreeId = typeof arrTreeId !== 'undefined' ? arrTreeId : [];
      option = option || {};
      await Promise.all(
        array.map(async child => {
          ...
          if (child.id) {
            const dataListChild = await Model.findAll(model, {
              where: { typeOfPostsParentId: child.id, ...option },
              attributes: ['id', 'typeOfPostsName', 'typeOfPostsParentId']
            });
            ...
            // console.log("arrayChild=======",arrayChild)
            await makeTreeArrayChildSearch_botTypeOfPosts(arrayChild, arrTreeId, model, option);
          }
        })
      );
...
function makeTreeParentChildrenArraySearch_botTypeOfPosts(array, arrTreeId, model, option) {
  return new Promise(async (resolve, reject) => {
    try {
      arrTreeId = typeof arrTreeId !== 'undefined' ? arrTreeId : [];
      option = option || {};
      ...
      await Promise.all(
        array.map(async child => {
          if (!_.includes(arrTreeId, child.id)) {
            ...
            if (child.typeOfPostsParentId || Number(child.typeOfPostsParentId) !== 0) {
              const data = await Model.findOne(model, {
                where: { id: child.typeOfPostsParentId, ...option },
                attributes: ['id', 'typeOfPostsName', 'typeOfPostsParentId']
              });
            ...
            await makeTreeParentChildrenArraySearch_botTypeOfPosts(arrayChild, arrTreeId, model, option);
          }
        })
      );
...
//           console.log('child.id====', child.id);
//           if (child.typeOfPostsParentId) {
//             await makeArraySearch([{ id: child.typeOfPostsParentId }], arrTreeId);
//           }
//         })
//       );
...
export default {
  ...
  combineFromDateWithToDate: (filter, option, field) => {
    ...
    if (filter[field[0]] && filter[field[1]]) {
      ...
      const fromtoDateFilter = _.assign(
        {},
        { [option]: { [Op.between]: [fromDate.toISOString(), toDate.toISOString()] } }
      );
  ...
  // typeOfNews
  makeTreeParentChildrenArraySearch_typeOfnews: async (array, arrTreeId, model, filterStatus) => {
    await makeTreeParentChildrenArraySearch_typeOfnews(array, arrTreeId, model, filterStatus);

    return arrTreeId;
  },
  makeTreeArrayChildSearch_typeOfnews: async (array, arrTreeId, model, option) => {
    await makeTreeArrayChildSearch_typeOfnews(array, arrTreeId, model, option);

    return arrTreeId;
  },
  makeTreeArrayParentSearch_typeOfnews: async (array, arrTreeId, model, option) => {
    await makeTreeArrayParentSearch_typeOfnews(array, arrTreeId, model, option);

    return arrTreeId;
  },
  // botTypeOfPosts
  makeTreeParentChildrenArraySearch_botTypeOfPosts: async (array, arrTreeId, model, filterStatus) => {
    await makeTreeParentChildrenArraySearch_botTypeOfPosts(array, arrTreeId, model, filterStatus);

    return arrTreeId;
  },
  makeTreeArrayChildSearch_botTypeOfPosts: async (array, arrTreeId, model, option) => {
    await makeTreeArrayChildSearch_botTypeOfPosts(array, arrTreeId, model, option);

    return arrTreeId;
  },
  makeTreeArrayParentSearch_botTypeOfPosts: async (array, arrTreeId, model, option) => {
    await makeTreeArrayParentSearch_botTypeOfPosts(array, arrTreeId, model, option);
