...
export const createTemplateUser = (workbook, { startRow, headerKey, headerTitle, reportName, param }) => {
  const worksheet = workbook.addWorksheet('Report', {
    pageSetup: { paperSize: 9, orientation: 'landscape', printTitlesRow: `${startRow}:${startRow}` ,fitToPage: true},
    views: [{ showGridLines: false, alignment: { wrapText: true, vertical: 'middle', horizontal: 'center' } }]
  });
...
export const createTemplateDisasters = (workbook, { startRow, headerKey, headerTitle, reportName, param }) => {
  const worksheet = workbook.addWorksheet('Report', {
    pageSetup: { paperSize: 9, orientation: 'landscape', printTitlesRow: `${startRow}:${startRow}` ,fitToPage: true},
    views: [{ showGridLines: false, alignment: { wrapText: true, vertical: 'middle', horizontal: 'center' } }]
  });
...
export const createTemplateExplorers= (workbook, { startRow, headerKey, headerTitle, reportName, param }) => {
  const worksheet = workbook.addWorksheet('Report', {
    pageSetup: { paperSize: 9, orientation: 'landscape', printTitlesRow: `${startRow}:${startRow}` ,fitToPage: true},
    views: [{ showGridLines: false, alignment: { wrapText: true, vertical: 'middle', horizontal: 'center' } }]
  });
...
export const createTemplateStatistics = (workbook, { startRow, headerKey, headerTitle, reportName, param }) => {
  const worksheet = workbook.addWorksheet('Report', {
    pageSetup: { paperSize: 9, orientation: 'landscape', printTitlesRow: `${startRow}:${startRow}` ,fitToPage: true},
    views: [{ showGridLines: false, alignment: { wrapText: true, vertical: 'middle', horizontal: 'center' } }]
  });
  ...
  if (param.FromDate && param.ToDate) {
    ...
    filterDateCell.value = `Từ ngày ${param.FromDate} - Đến ngày ${param.ToDate}`;
    filterDateCell.font = font;
    filterDateCell.alignment = alignment;
  }
  worksheet.mergeCells(`B5:F5`);
...
export const parseSortMongodb = (sort) => {

    // sort = (typeof sort === 'string') ? JSON.parse(sort).map((e, i) => i % 2 === 0 ? e : e) : defaultValue;
    sort = _.replace(sort,'[','{');
    sort = _.replace(sort,']','}');
    sort = _.replace(sort,',',':');
    sort = _.replace(sort,'desc','-1');
    sort = _.replace(sort,'asc','1');
    sort = _.replace(sort,'DESC','-1');
    sort = _.replace(sort,'ASC','1');
...
export const parseSortVer2 = (sort,table) => {
  ...
  try {
    sort = (typeof sort === 'string') ? JSON.parse(sort).map((e, i) => i % 2 === 0 ? sequelize.col(`${table}.${e}`) : e) : defaultValue;
    if (sort.length % 2 === 0) {
      sort = _.chunk(sort,2) || defaultValue;
      sort.map(element => {
        if (!element[1].match(regexSort)) {
...
export const simplifyName = (_name) => {
  _name = _name.trim().toLocaleLowerCase().replace(/\s+/g, " ");
  _name = _name.replace(/[àáạảãâầấậẩẫăằắặẳẵ]/g, "a");
  _name = _name.replace(/[èéẹẻẽêềếệểễ]/g, "e");
  _name = _name.replace(/[ìíịỉĩ]/g, "i");
  _name = _name.replace(/[òóọỏõôồốộổỗơờớợởỡ]/g, "o");
